...
what do we mean by developer self-service?
using software templates for developer self-service

use case: t-shirt size namespaces + middleware components to be deployed in a namespace

introduce the orchestrator plugin: allows to execute workflows from backstage. Powered by Serverless Workflow.

The orchestrator plugin is integrated with the notification plugin. This allows for a workflow to send notifications to a user or group in backstage at particular stages in the execution of the workflow.
