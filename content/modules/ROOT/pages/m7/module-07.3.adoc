...
=== Refresh the `backstage` ArgoCD application

To apply the changes

. Navigate to view the https://openshift-gitops-server-openshift-gitops.{openshift_cluster_ingress_domain}/applications[`backstage` ArgoCD Application in OpenShift GitOps^, window="argo"]. Login as `admin/{openshift_gitops_password}` if prompted.
. Click the *Refresh* button for the `backstage` ArgoCD application.
. Wait until the Application status is *Healthy*.
. A a new `servicemonitor` for {product_name_rhdh} should now be created. You can verify this by checking the `backstage` ArgoCD application. 
+
