...
In this module, you will import an existing API and an existing service as Components by using the Software Templates that have been setup by Platform Engineers.
...
*NOTE*: A number of these fields have been prepopulated with default values for convenience only for workshop purposes. In real life, developers will need provide almost all of the values that are needed to import existing apps/APIs.
