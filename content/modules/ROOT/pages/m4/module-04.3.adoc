...
It's time to use the *Import API to Existing Catalog Envelope* Template that you registered earlier. Doing so will allow this API to be associated with the `parasol-store` Component.

. Select *Create* from the {product_name_rhdh} side menu.
. Click the *Choose* button on the *Import API to Existing Catalog Envelope* Template.
+
image:m4/import-api-template.png[width=30%]
. You will be presented with the Software Templates wizard.
.. *Step 1*: Provide information about Catalog Envelope to which API will be added. Fill out the following values and click *Next*.
+
...
|====
| *Field* | *Description* | *Value*
| GitLab Organization | Organization where the repository resides| `parasol`
| Repository Name | Repository containing source code | `parasol-store-api` 
