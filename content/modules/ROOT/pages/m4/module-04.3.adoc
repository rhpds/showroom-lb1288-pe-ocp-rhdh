...
. You can navigate to the https://gitlab-gitlab.{openshift_cluster_ingress_domain}/parasol/all-location-parasol[all-location-parasol repo, window="gitlab"] to see that a new folder named `parasol-store-api` has been creates with a `catalog-info.yaml` and a docs folder for `TechDocs`.
+
