...
In this module, you will import an existing API and an existing Service using the Software Templates that were setup as a Platform Engineer in the previous section.

IMPORTANT: Please make sure to log in as a *Developer* with `dev1/{common_password}`. If you are unsure, please refer to the below note.
...
. From the *Create* menu on the left-hand of {product_name_rhdh}, click on the *Choose* button of the `Import Existing Application` template.
+
...
. *CD Tab* displays the deployed components/systems using ArgoCD plugin
+
